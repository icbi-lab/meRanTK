...
if ( $allowDoveTail && ($supportedStarVersions{$starVersion} >= 4)) {
    $run_opts{star_alignEndsProtrude} = [ $dtMaxBases, 'ConcordantPair' ];
}
if ( $allowDoveTail && ($supportedStarVersions{$starVersion} < 4)) {
    say STDERR "Allowing dovetail (-dt) reads works only with STAR version >= 2.6.1d";
    exit(1);
}
...
Options:
    ...
    -dtMaxBases|dtmb      : If "dovetailing": maximum number of protrusion bases allowed
                            (default: 20 - only if -dt is set)
