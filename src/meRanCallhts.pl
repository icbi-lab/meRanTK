...
if ( !-r $baidxFile ) {
    if ( checkWirteToDir($fpath) ) {
        $bai = $sam->hts_index;    # try to open the bam index, should be recreated if this fails.
        undef($bai);
    }
    else {
...
Total analyzed C to " . (($azaMode) ? "G" : "C") . " conversion rate:\t" . $totalConvRate_analyzed . "
...
Total C to " . (($azaMode) ? "G" : "C") . " conversion rate estimated:\t" . $totalConvRate_estimated . "
...
sub sam_to_bam {
    ...
    my $tam = Bio::DB::HTSfile->open($sam) || die("Could not open SAM file for reading: $!");
    my $header = $tam->header_read();

    my $out = Bio::DB::HTSfile->open( $bam, 'wc' ) || die("Could not open BAM file for writing: $!");
    $out->header_write($header, $fastaInFile);
    #$out->header_write($header);

    # my $alignment = Bio::DB::HTS::Alignment->new();
    my $lines     = 0;

    while ( my $alignment = $tam->read1( $header )  ) {
        # print STDERR Dumper($alignment);
        $out->write1($header, $alignment);
...
sub sort_bam {
    ...
    Bio::DB::BAMSORT->sort_core( 0, $bam, $sorted, 1000000000 );
...
sub index_bam {
    ...
    Bio::DB::HTSfile->index_build($bam);
    return;
}
